...
class TestTaskUpdate:
    def test_task_update(self, steps: Steps, task_facade: TaskFacade) -> None:
        task = steps.create_task()
        new_title = "new title"
        new_deadline = datetime.now(timezone.utc) + timedelta(days=2)

        updated_task = task_facade.update_by_id(
...
class TestTaskComplete:
    def test_task_complete(self, steps: Steps, task_facade: TaskFacade) -> None:
        task = steps.create_task()

        updated_task = task_facade.change_task_state(id=task.id, completed=True)
    ...
    def test_task_complete_fails_on_already_completed(
        ...
        task_facade.change_task_state(id=task.id, completed=True)
        with pytest.raises(TaskWrongStateException):
            task_facade.change_task_state(id=task.id, completed=True)

    def test_task_uncomplete(self, steps: Steps, task_facade: TaskFacade) -> None:
        task = steps.create_task()

        task_facade.change_task_state(id=task.id, completed=True)
        updated_task = task_facade.change_task_state(id=task.id, completed=False)
    ...
    def test_task_uncomplete_fails_on_already_uncompleted(
        ...
        with pytest.raises(TaskWrongStateException):
            task_facade.change_task_state(id=task.id, completed=False)
    ...
    def test_task_change_state_fails_on_invalid_id(
        ...
        with pytest.raises(TaskNotFoundException, match=str(invalid_uuid)):
            task_facade.change_task_state(id=invalid_uuid, completed=True)
