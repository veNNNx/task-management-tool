...
class Steps:
    ...
    def update_task_by_id(self, id: UUID, title: str, deadline: datetime) -> Task:
        return self._task_service.update_by_id(id=id, title=title, deadline=deadline)

    def change_task_state(self, task_id: UUID, completed: bool = True) -> None:
        self._task_service.change_task_state(id=task_id, completed=completed)

    def delete_task_by_id(self, task_id: UUID) -> None:
        self._task_service.delete_by_id(task_id)
